...
export class SettingsManager {
    ...
    private getHtml(): string {
        ...
        <style>
            ...
            * {
            ...
            :root {
                ...
                --accent: #1db954;
                --accent-hover: #1ed760;
                --accent-muted: rgba(29, 185, 84, 0.6);
                --link-color: #1db954;
                --link-hover: #1ed760;
            ...
            input[type=range].styled-slider {
                ...
                width: 100%;
                height: 4px;
                background: linear-gradient(to right, var(--accent) 0%, var(--accent) var(--slider-progress, 0%), var(--border) var(--slider-progress, 0%), var(--border) 100%);
                border-radius: 2px;
                outline: none;
                opacity: 0.7;
        ...
        <div class="settings-panel">
            <div class="setting-group">
                ...
                <div class="setting-item">
                    <span data-i18n="enableSplash">${this.translationService.translate('enableSplash')}</span>
                    <label class="toggle">
                        <input type="checkbox" id="splashEnabled" ${this.store.get('splashEnabled', true) ? 'checked' : ''}>
                        <span class="slider"></span>
                    </label>
                </div>
                <div class="setting-item">
            ...
            <div class="setting-group">
                ...
                <div class="setting-item">
                    ...
                    <select id="bypassModeSelector" class="theme-selector">
                        <option value="none" data-i18n="bypassNone">${this.translationService.translate('bypassNone')}</option>
                        <option value="dns" data-i18n="bypassDns">${this.translationService.translate('bypassDns')}</option>
                        <option value="zapret" data-i18n="bypassZapret">${this.translationService.translate('bypassZapret')}</option>
                    </select>
                </div>

                <!-- DNS Settings -->
                <div id="dnsSettings" style="display: none; margin-top: 10px;">
                    <div class="setting-item">
                        <span data-i18n="dnsPreset">${this.translationService.translate('dnsPreset')}</span>
                        <select id="dnsPreset" class="theme-selector">
                            <option value="" data-i18n="dnsPresetNone">${this.translationService.translate('dnsPresetNone')}</option>
                            <option value="1.1.1.1" data-i18n="dnsPresetCloudflare">${this.translationService.translate('dnsPresetCloudflare')}</option>
                            <option value="8.8.8.8" data-i18n="dnsPresetGoogle">${this.translationService.translate('dnsPresetGoogle')}</option>
                            <option value="custom" data-i18n="dnsPresetCustom">${this.translationService.translate('dnsPresetCustom')}</option>
                        </select>
                    </div>
                    <input type="text" class="textInput" id="dnsAddress" placeholder="${this.translationService.translate('dnsAddress')}" data-i18n-placeholder="dnsAddress" value="${this.store.get('dnsAddress') || ''}" style="display: none; margin-top: 10px;">
  ...
  "originUrl": "https://soundcloud.com/track-url"
            ...
            <div class="setting-group">
                ...
                <div class="setting-item">
                    <span>Keep theme active in background<br>(requires restart)</span>
                    <label class="toggle">
                        <input type="checkbox" id="themeAlwaysActive" ${this.store.get('themeAlwaysActive', false) ? 'checked' : ''}>
                        <span class="slider"></span>
                    </label>
                </div>
                <div class="description">WARNING: Enabling this may increase CPU/GPU usage.</div>

                <div class="setting-item">
                    <span>Background Blur</span>
                    <div class="slider-container">
                        <input type="range" id="backgroundBlur" class="styled-slider" min="0" max="20" step="1" value="${this.store.get('backgroundBlur', 0)}">
                        <span id="backgroundBlurValue">${this.store.get('backgroundBlur', 0)}px</span>
                    </div>
                </div>
            </div>
    ...
    private getJavaScript(): string {
        return `
            ...
            // Bypass Settings
            const bypassModeSelector = document.getElementById('bypassModeSelector');
            const dnsSettings = document.getElementById('dnsSettings');
            const zapretDescription = document.getElementById('zapretDescription');
            const dnsPresetSelect = document.getElementById('dnsPreset');
            const dnsAddressInput = document.getElementById('dnsAddress');

            function setBypassUIState() {
            ...
            function setDnsUIState() {
                const preset = dnsPresetSelect.value;
                const isCustom = preset === 'custom';
                dnsAddressInput.style.display = isCustom ? 'block' : 'none';
                dnsAddressInput.disabled = !isCustom;

                if (!isCustom) {
                    dnsAddressInput.value = preset;
                    ipcRenderer.send('setting-changed', { key: 'dnsAddress', value: preset });
                }
            }

            // Initial State
            document.addEventListener('DOMContentLoaded', async () => {
                const initialMode = await ipcRenderer.invoke('get-store-value', 'bypassMode') || 'none';
                bypassModeSelector.value = initialMode;
                setBypassUIState();

                const initialDnsAddress = await ipcRenderer.invoke('get-store-value', 'dnsAddress');
                if (initialDnsAddress === '1.1.1.1' || initialDnsAddress === '8.8.8.8') {
                    dnsPresetSelect.value = initialDnsAddress;
                } else if (initialDnsAddress) {
                    dnsPresetSelect.value = 'custom';
                    dnsAddressInput.value = initialDnsAddress;
                } else {
            ...
            // Event Listeners
            bypassModeSelector.addEventListener('change', () => {
                const mode = bypassModeSelector.value;
                ipcRenderer.send('setting-changed', { key: 'bypassMode', value: mode });
                setBypassUIState();
            });

            dnsPresetSelect.addEventListener('change', () => {
                setDnsUIState();
                if (dnsPresetSelect.value === 'custom') {
                    dnsAddressInput.value = '';
                    ipcRenderer.send('setting-changed', { key: 'dnsAddress', value: '' });
                }
            });

            dnsAddressInput.addEventListener('input', (e) => {
                ipcRenderer.send('setting-changed', { key: 'dnsAddress', value: e.target.value });
            });
            ...
            document.getElementById('splashEnabled').addEventListener('change', (e) => {
                ipcRenderer.send('setting-changed', { key: 'splashEnabled', value: e.target.checked });
            });
            ...
            document.getElementById('themeAlwaysActive').addEventListener('change', (e) => {
                ipcRenderer.send('setting-changed', { key: 'themeAlwaysActive', value: e.target.checked });
            });

            document.getElementById('backgroundBlur').addEventListener('input', (e) => {
                const slider = e.target;
                const blurValue = slider.value;
                const progress = (blurValue / slider.max) * 100;

                slider.style.setProperty('--slider-progress', \`\${progress}%\`);
                document.getElementById('backgroundBlurValue').textContent = \`\${blurValue}px\`;
                ipcRenderer.send('setting-changed', { key: 'backgroundBlur', value: blurValue });
            });
